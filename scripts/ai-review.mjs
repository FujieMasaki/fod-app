...
export async function review({ env = process.env, fetchImpl = fetch } = {}) {
  if (!/^[1-9]\d{0,8}$/.test(env.PR_NUMBER ?? "")) {
    throw new Error("PR_NUMBER must be a positive integer.");
  }
  if (!/^[\w.-]+\/[\w.-]+$/.test(env.GITHUB_REPOSITORY ?? "")) {
    throw new Error("GITHUB_REPOSITORY is invalid.");
  }
  if (!env.OPENAI_API_KEY || !env.GH_TOKEN) {
    throw new Error("Set the OPENAI_API_KEY repository secret before running AI Review.");
  }
  ...
  async function github(url, { accept = "application/vnd.github+json", body } = {}) {
    ...
    // Do not log response bodies: these can contain private code or credentials.
  ...
  // One request, no automatic retries, no tools, and no PR code execution.
  ...
  if (result.status !== "completed") {
    throw new Error("OpenAI response incomplete; no partial review posted. The request may still be billed.");
  }
  ...
  // Render model output as plain text to prevent active images/HTML/mentions.
  ...
  if (env.GITHUB_STEP_SUMMARY) {
    appendFileSync(env.GITHUB_STEP_SUMMARY, `Reviewed PR #${env.PR_NUMBER} at ${pr.head.sha}. ${usageText}\n`);
  }
  return usage;
}
